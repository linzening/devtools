...
class Ipdbv6func {
	...
	public $offlen;
	public $iplen;
	private function __construct($file="./uploads/mmdb/ipv6wry.db", $dbipv4=null) {
		if (PHP_INT_SIZE < 8) {
	 ...
	 * 范围：1-138649
	 */
	public function querylist($ip_find = 42151){
		$ip_offset = $this->index_start_offset + $ip_find * ($this->iplen + $this->offlen);
		$ip_offset2 = $ip_offset + $this->iplen + $this->offlen;
		$ip_start = inet_ntop(pack("J2", $this->read8($ip_offset), 0));
		try {
		...
		$ip_record_offset = $this->read8($ip_offset + $this->iplen, $this->offlen); //固定值
		$ip_addr = $this->read_record($ip_record_offset);
		$ip_addr_disp = $ip_addr[0] . " " . $ip_addr[1];
		return [
			'start' => $ip_start,
	...
	public function getIndex($ip){
		...
		$ip_num1 = $ip_num_arr[1];
		$ip_num2 = $ip_num_arr[2];
		return $this->find($ip_num1, $ip_num2, 0, $this->total);
	}
